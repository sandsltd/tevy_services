...
import { X, ChevronRight, Car, Wrench, Calendar, Clock, HelpCircle } from 'lucide-react'
...
export default function ServiceBooking({ 
  ...
  const renderStep = () => {
    switch (step) {
      ...
      case 2:
        if (serviceDetails.serviceTypes.includes('tyre-replacement')) {
          return renderTyreDetailsStep()
        }
        return (
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              {/* Number of wheels */}
              <div>
                <label className="block text-sm text-gray-400 mb-2">Number of Wheels</label>
                  ...
                  {getAvailableServices().map((service) => (
                    <button
                      ...
                      onClick={() => setServiceDetails(prev => {
                        const newTypes = prev.serviceTypes.includes(service.id)
                          ? prev.serviceTypes.filter(type => type !== service.id)
      ...
      case 3:
        return needsPhotos ? (
          ...
          // Render contact form directly if no photos needed
          <ContactForm />
        )
  ...
  const renderTyreDetailsStep = () => {
    const isWorkshop = serviceType === 'workshop'

    return (
      <div className="space-y-4">
        <h4 className="font-semibold">Tyre Details</h4>
        <p className="text-sm text-gray-400 mb-4">
          Help us provide an accurate quote by providing some details about your tyres
        </p>
        ...
        {/* Number of Tyres */}
        <div>
          <label className="block text-sm text-gray-400 mb-2">How many tyres need replacing?</label>
